...
* 单例类
	...
	|**			|区别												|
	|-----------|:-------------------------------------------------:|
	|饿汉式		|空间换时间。在多线程访问时不会创建多个对象			|
	|懒汉式		|时间换空间。在多线程访问时有可能创建多个对象		|
	...
	class Singleton {
		...
		private Singleton(){}
		//2.创建本类对象
		private static Singleton h = new Singleton();
		//3.对外提供公共的访问方法
...
* Timer 
	...
	* 指定时间执行指定任务。执行一次或定期重复执行
	...
	t.schedule(new TimerTask() {
		public void run() {
			System.out.println("间隔执行的语句");
		}
	}, new Date(2016 - 1900, 2, 10, 13, 25, 0), 5000);
	```
* 在同步代码块中，用哪个对象锁，就用哪个对象调用wait()和notify()
* 为什么wait() 和notify() 定义在Object类中？
	* 因为锁对象可以是任意对象，Object是所有类的父类，所以wait() 和notify() 需要定义在在Object中

	
	|**			|				参数					|				位置			|   锁		|
	|-----------|:-------------------------------------:|:-----------------------------:|:---------:|
	|sleep(...)	|必须传入参数。参数就是时间，时间到了自动醒来|在同步函数或同步代码块中	|不释放锁	|
	|wait()		|参数可传可不传。传入参数就是在参数的时间结束后等待，不传就是直接等待|在同步函数或同步代码块中|释放锁|

* ReentrantLock 
	* 互斥锁
	* JDK 1.5 新特性
	```java
	public class Demo3_ReentrantLock_mytrial {

		public static void main(String[] args) {
			final Lobster l = new Lobster();
			new Thread() {
				public void run() {
					while (true) {
						try {
	...
	class Lobster {
		...
		public void lob1 () throws InterruptedException {
			...
			flag = 2; 
			//唤醒lob2
			c2.signal();	// 唤醒指定的线程，将c2唤醒
			r.unlock(); 	//释放锁
		}
		
		public void lob2 () throws InterruptedException {
			//获取锁
			r.lock();
			if (flag != 2) {
				c2.await();			//让c2等待
		...
		public void lob3() throws InterruptedException {
			//获取锁
			r.lock();
			//判断是否使其等待
			if (flag != 3) {
...
* 线程池
	...
	* 程序启动一个新线程的成本是比较高的，因为它涉及到要与操作系统进行交互。而是用线程池可以跟好的提高性能，尤其是当程序中要创建大量生存周期很短的线程时，更应该考虑使用线程池。
...
* 设计模式
	...
	* 工厂方法模式
		...
		* 缺点：需要额外的编写代码，增加了工作量
	* 适配器设计模式 
		* 适配器就是一个类，实现了监听器接口，所有抽象方法都重写了，但方法是空的
		* 适配器类需要定义成抽象的，因为创建该类对象，调用空方法是没有意义的
		* 目的就是为了简化程序员的操作，定义监听器时继承适配器，只重写需要的方法就可以了
* GUI
	|布局管理器			|内容									    				|
	|-------------------|:---------------------------------------------------------:|
	|FLowLayout(流式)	|从左到右的顺序排列，总是居中，Panel默认的布局管理器		|
	|BorderLayout(边界)	|分为东、西、南、北、中。Frame默认的布局管理器				|
	|GridLayout(网格)	|规则的矩阵	                                                |
	|CardLayout(卡片)	|选项卡                                                     |
	|GridBagLayout(网格包)|非规则的矩阵                                             |

	* 适配器
		* 在使用监听器的时候，需要定义一个事件监听器接口
		* 通常接口中有多个方法，而程序中不一定所有的都用到，但有必须重写，很繁琐
		* 适配器简化了这些操作，定义监听器时只要继承适配器，然后重写需要的方法即可
	* 事件处理
		* 事件：用户的一个操作
		* 事件源：被操作的组件
		* 监听器：一个自定义的对象，实现了监听器接口，包含事件处理方法，把监听器添加在事件源上，当事件发生的时候JVM就会自动调用监听器中的事件处理方法
...
* 端口号
	...
	* 常用端口：
		...
		* feiQ : 2425
* 协议
	* 为计算机网络中记性数据交换而建立的规则、标准或约定的集合
	* UDP
		* 面向无连接。数据不安全。速度快。不区分客户端和服务端
	* TCP
		* 面向连接（3次握手）。数据安全。速度略低。分为客户端和服务端
			* 3次握手：客户端先向服务端发起请求，服务端响应请求，传输数据
...
* Socket（套接字）
	...
	* 网络通信其实就是Socket贱的通信
	* 数据在两个Socket间通过IO流传输
	* Socket在应用程序中创建，通过一种绑定机制与去顶程序建立关系，告诉自己所对应的IP和port

## 反射
* 类的加载概述
	* 当成需要使用某个类时，如果该类还未被加载到内存中，则系统会通过加载、连接、初始化散步来实现对这个类进行初始化
	* 加载
		* 是指将class文件读入内存，并为之创建一个Class对象。任何类被使用时系统都会建立一个Class对象
	* 连接
		* 1.验证	是否有正确的内部结构，并和其他类协调一致
		* 2.准备	负责为类的静态成员分配内存，并设置默认初始化
...
* 类加载器
	...
	* 分类 & 作用
	...
	|分类					|名称				|作用							|
	|-----------------------|:-----------------:|:-------------------------------------------:|
	|Boostrap ClassLoader	|根类加载器（引导类加载器）|负责Java核心类的加载|
	|Extension ClassLoader	| 扩展类加载器		|负责JRE的扩展目录中jar包的加载|
	|System ClassLoader		|系统类加载器		|负责在JVM启动时记载来自java命令的class文件，以及classpath环境变量所指定的jar包和类路径|
...
* 反射
	...
	|反射的3种方式|Class clazz = Class.forName("类名")|Class clazz = Person.class|Class clazz = p.getClass()|
	|作用	|对区配置文件			|当做静态方法的随对象		|判断是否是同一个字节码对象|

	* Class类的newInstance()是通过 使用该类的无参的构造函数创建对象，如果一个类没有无参的构造函数，就不能这样创建了，可以调用Class类的getConstructor()
...
* 枚举类的注意事项
	* 定义枚举类要用关键字 enum
	* 所有枚举类都是Enum 的子类
	* 枚举类的第一行必须是枚举项，最后一个枚举项后的分号是可以省略的，但是入伙枚举类有其他的东西，这个分号就不能省略，建议不要省略
	* 枚举类可以有构造器，但必须是用private，且默认也是private修饰
